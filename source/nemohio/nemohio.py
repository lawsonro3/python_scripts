...
This moduel provies an interface to the command line Nemoh code
"""
import numpy as np
...
class NemohResults(object):
    '''
    Class that is used to read and process Nemoh simulation results
    '''
    def __init__(self,baseDir):
        self.baseDir = baseDir
        self.dir = os.path.join(self.baseDir,'results')
  
        self.freq = [] # need to clean up so this is consistant with the sim class data
        self.addedMass = {}
        self.radiationDamping = {}

        self.files = {}        
        self.files['index.dat'] =               os.path.join(self.dir,'index.dat')
        self.files['Force.dat'] =               os.path.join(self.dir,'Force.dat')
        self.files['FKForces.dat'] =            os.path.join(self.dir,'FKForces.dat')
        self.files['Fe.dat'] =                  os.path.join(self.dir,'Fe.dat')
        self.files['CM.dat'] =                  os.path.join(self.dir,'CM.dat')
        self.files['CA.dat'] =                  os.path.join(self.dir,'CA.dat')
        self.files['Normalvelocities.dat'] =    os.path.join(self.dir,'Normalvelocities.dat')
    ...
    def readCMCA(self):
        ...
        for i in xrange(self.numFreqs):
            self.freq.append(float(linesCM[1+i*7].replace('\n','')))
    ...
    def plotAddedMassAndDamping(self):
        '''
        Function to plot the diagional component of the added mass and radiation
        damping coeffciient 
        Inputs:
            None
        Outputs:
